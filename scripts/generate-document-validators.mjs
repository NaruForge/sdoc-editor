...
// Both modes use exactly the same schema/options except for error collection.
// Detailed output remains the diagnostic oracle; fast output only decides validity.
for (const allErrors of [true, false]) {
  const basename = allErrors ? 'documentValidators' : 'fastDocumentValidators';
  const outputPath = resolve(root, `shared/document/generated/${basename}.js`);
  const declarationPath = resolve(root, `shared/document/generated/${basename}.d.ts`);

  const ajv = new Ajv({ allErrors, strict: false, code: { source: true, esm: true } });
  addFormats(ajv);
  ...
  if (process.argv.includes('--check')) {
    ...
    console.log(`Generated CSP-safe ${basename}.`);
  }
}
